...
/**
 * 出典（2026-08-01 に公式DBと突き合わせて確定）
 *
 * アプリ側の source フィールドには版が記録されていなかったため、文部科学省の
 * 食品成分データベース（https://fooddb.mext.go.jp/）の収載値と実測比較して特定した。
 *
 *   水稲めし 精白米  八訂増補2023年 156kcal / アプリDB 168kcal / 七訂 168kcal
 *   角形食パン 食パン 八訂増補2023年 248kcal・脂質4.1・炭水化物46.4
 *                    アプリDB      264kcal・脂質4.4・炭水化物46.7（＝七訂と一致）
 *
 * 八訂は組成成分によるエネルギー計算へ変更され主食類のkcalが下がっている。
 * アプリDBは下がる前の値なので【七訂】で確定。
 *
 * ただし一部に微差がある（例: 食パンのたんぱく質 七訂9.3 / アプリDB9.0）ため、
 * 「基に収録した値」「収載値と異なる場合がある」という表現を崩さないこと。
 */
const FOOD_DATA_SOURCE = '文部科学省『日本食品標準成分表2015年版（七訂）』';

/**
 * 公開対象とする source（これ以外は生成しない）
 *
 * ★ 重要: アプリの食品DB695件のうち、出典は次のように混在している。
 *     日本食品標準成分表 375 / カロリーSlism 282 / 一般的な値 35 / メーカー公式 3
 *
 *   全件を「日本食品標準成分表」として公開すると46%が虚偽表示になる（景表法）。
 *   加えて、他サイトが編集した成分データを大量に再掲するのは著作権・データベース権の
 *   リスクがあり、「一般的な値」は検証不能で出典として成立しない。
 *   よって【公的機関の出典が明確な375件だけ】を公開する。
 *
 *   対象を広げたい場合は、各ページに実際のsourceを個別表示し、権利関係を確認してから行うこと。
 */
const ALLOWED_SOURCE = '日本食品標準成分表';
...
function sourceBlock() {
  return `      <section class="food-source">
        <h2>出典と注意事項</h2>
        <p>栄養成分値は${esc(FOOD_DATA_SOURCE)}を基に、プロテインノートの食品データベースへ収録した値です。収録の過程で数値を丸めている場合があり、成分表の収載値と一致しないことがあります。</p>
